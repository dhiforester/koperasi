<?php 
    ...
    if(empty($_POST['KeywordByExport'])){
        $KeywordByExport=$_POST['KeywordByExport'];
    }else{
        $KeywordByExport=$_POST['KeywordByExport'];
    }
    if(empty($_POST['FilterKeywordExport'])){
        $keyword=$_POST['FilterKeywordExport'];
    }else{
        $keyword=$_POST['FilterKeywordExport'];
    }
    if(empty($KeywordByExport)){
        if(empty($keyword)){
        ...
        if(empty($keyword)){
            ...
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM pinjaman WHERE $KeywordByExport like '%$keyword%'"));
        }
    }
?> 
    <html>
                    ...
                    if(empty($jml_data)){
                        ...
                        if(empty($KeywordByExport)){
                            if(empty($keyword)){
                                ...
                                $query = mysqli_query($Conn, "SELECT*FROM pinjaman WHERE tanggal_pinjaman like '%$keyword%' OR tanggal_input like '%$keyword%' OR nama like '%$keyword%' OR jumlah_pinjaman like '%$keyword%' OR nilai_angsuran like '%$keyword%' OR periode_angsuran like '%$keyword%' OR status like '%$keyword%'");
                            }
                        }else{
                            if(empty($keyword)){
                                $query = mysqli_query($Conn, "SELECT*FROM pinjaman");
                            }else{
                                $query = mysqli_query($Conn, "SELECT*FROM pinjaman WHERE $KeywordByExport like '%$keyword%'");
                            }
                        }
